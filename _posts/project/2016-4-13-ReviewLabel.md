...
具体的[参数更新推导][9]。同样的词向量生成算法还有[GloVe][10]。
...
通过这些门，模型有能力学习到何时该忘记，或是重新训练关于序列的特定信息。在本文提出的模型中，将GRU应用于文本中，词向量作为$$t$$时刻的输入，产生最终的隐层输出层$$h_f$$。最后将这个向量输入一个全连接层，输出层大小为$$|L|$$，来产生最后的输出$$\hat y$$，其中第$$i$$个元素代表标签$$i$$被赋予当前评论的概率。

...
本文首先对从应用商店爬取的用户评论进行人工分析，提炼出四种主题作为分类标签：故障报告，功能需求，用户体验，感性评价。之后，针对评论文本的多标签分类任务，本文设计并实现了一套自动化标记评论文本的工具：该工具通过词向量来表征评论文本，作为学习模型的输入，进而使用神经网络模型在训练集上进行学习。我们分别实现了卷积神经网络模型和门限循环网络模型，神经网络输出层即为文本被分为不同标签的概率，可人为设定阈值进行判断。最后，我们通过在测试集上进行实验评估并比较其最终分类效果。

# 2. 数据集 

通过对评论内容的人工分析，我们将评论分为四个主题标签：故障报告、功能需
求、用户体验和感性评价。一条评论可能包含许多不同的事件类型。比如，
...
\textbf{形符化}是将不同词形还原到词元本身的过程，例如“fixing”“fixed”“fixes”都来源于“fix”。类似地，\textbf{归一化}指的是去除单词后缀来还原词的基础形式，如“window”“Window”“windows”都对应“window”。词形还原考虑语言学上下文并使用词典，而词干提取只是对单个的词操作，无法根据词性区分词的不同涵义。比如词形还原中会将“better”还原为“good”，而词干提取会把“goods”和“good”一视同仁。词形还原和词干提取都能帮助统一具有意义相同、词形不同的关键词，从而缩小词典大小。我们使用NLTK的PorterStemmer从英文单词中获得符合语法的（前缀）词干。
...
CNN 和 RNN 在许多NLP 任务中都有良好表现，因此本文分别实现这两种模型用于多标签评论的分类任务。
...
本文使用的CNN模型如下图所示，共分四层，第一层是词向量层，即我们为每个评论文本构造的$n$行$k$列矩阵；第二层是卷积层，多个filter作用于词向量层，不同filter生成不同的`feature map`；第三层是采样层，取每个feature map的最大值；第四层是一个全连接的softmax层，输出是每个标签的概率。除此之外，输入层可以有两个channel，其中一个channel采用预先利用word2vec训练好的词向量，另一个channel的词向量可以通过\gls*{BP}在训练过程中调整。而这里使用的是静态的词向量。
...
通过这些门，模型有能力学习到何时该忘记，或是重新训练关于序列的特定信息。在本文提出的模型中，将\gls*{GRU}应用于文本中，词向量作为$t$时刻的输入，产生最终的隐层输出层$h_f$。最后将这个向量输入一个全连接层，输出层大小为$|L|$，来产生最后的输出$\hat y$，其中第$i$个元素代表标签$i$被赋予当前文本的概率。
